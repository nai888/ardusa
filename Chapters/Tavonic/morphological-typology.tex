...
\pex<ex:tvk-iso-nouns>
	...
	\endgl
	\a<phrase>\begingl
		\glpreamble\fw{Eson moþes elbi šus ken botra draš.}\\
...
\pex<ex:tvk-noun-clitics>
	...
	\a<possessednoun>\begingl
		\glpreamble\fw{Mod nas su eson urda oko fra.}\\
...
This phonological reduction occurs no matter whether the following word is the noun the preposition is modifying or not. For example, notice in example~\getfullref{ex:tvk-clitic-reduction.su} that the preposition attaches itself to \fw{urda} even though it is an adjective modifying the noun \fw{ablu}.
...
In example~\getfullref{ex:tvk-depmarking-possessive.pn}, \defn{you} are grammatically in possession of \fw{botra} \defn{wife}; the possessee forms the head of the phrase while it is modified by the possessor, which receives the genitive inflection. In example~\getfullref{ex:tvk-depmarking-possessive.n}, \fw{botra} is still the possessee and thus the head of the phrase while the genitive is marked on the dependent, \fw{esobiš} \defn{patriot}, using a noun preposition.
...
\ex<ex:tvk-depmarking-adjective>
	\remainex\begin{minipage}[t]{0.5\remaining}
		\begingl
			\glpreamble\fw{ran ǩalonža botra}\\
	...
	\begin{minipage}[t]{0.5\remaining}
		\begin{forest} dotted tier
			[\fw{botra}
				[\fw{\textbf{ran}}, name=prepmod
					[\fw{\textbf{ran}}, name=prep]
				]
				[\fw{ǩalonž\textbf{a}}, name=adj
...
In example~\getref{ex:tvk-depmarking-adjective}, while the head noun \fw{botra} is marked for animacy with \fw{ran}, the dependent modifying adjective \fw{ǩalonža} \defn{husbandless} takes the \fw{-a} animate ending to match.
...
\pex<ex:tvk-depmarking-verbs>
	...
	\end{minipage}
	\a<transn>\begingl
		\glpreamble\fw{Do su botra ǩalo mi tavofre alma frak.}\\
...
In example~\getfullref{ex:tvk-depmarking-verbs.intranspn}, the pronoun \fw{mon} is declined to indicate it is the topic of the sentence while the verb \fw{usuš}, despite conjugating for mood, tense, and aspect, is not marked for this role. Example~\getfullref{ex:tvk-depmarking-verbs.transpn} similarly marks the two pronouns \fw{mon} and \fw{þet} for their roles in the sentence as topic and object while the verb \fw{kante} does not inflect to indicate these roles. When nouns are used instead of pronouns, as in examples~\getfullref{ex:tvk-depmarking-verbs.intransn}--\getref{ex:tvk-depmarking-verbs.transn}, the nouns are marked for their grammatical role by their prepositions, their own dependents, while the head verb remains unmarked for these roles.
