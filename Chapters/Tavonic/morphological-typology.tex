...
\index[tavonic]{morphological typology|(}
...
\index[tavonic]{morphological typology!fusion|(}
...
\pex<ex:tvk-concat-prons>
	\a<fpsa> \scr{mor} \fw{mor} \phnm{mor} \defn{I} \gloss{\Fps.\Abs}
	\a<sppa> \scr{þeton} \fw{þeton} \phnm{θe\pstrs ton} \defn{you} \gloss{\Spp.\Acc}
	\a<tpcitd> \scr{ginsek} \fw{ginsek} \phnm{gin\pstrs sek} \defn{to it} \gloss{\Tpc.\In.\Top.\Dat}
\xe

This concatenation appears not only in inflectional morphology, but also in derivational morphology. For example, the word \scr{ablutik} \fw{ablutik} \phnm{a.blu\pstrs tik} \defn{kitten} is formed from the root noun \scr{ablu} \fw{ablu} \phnm{a\pstrs blu} \defn{cat} with a diminutive suffix attached \gloss{\fw{ablu}-\Dim}. Similarly, the word \scr{akradir} \fw{akradir} \phnm{ak.ra\pstrs dir} \defn{pen} is formed from the root verb \scr{akrali} \fw{akrali} \phnm{ak\pstrs ra.li} \defn{to write} with a nominalizing suffix \gloss{\fw{akra}-\Nmz}.
...
\pex<ex:tvk-iso-nouns>
	\a<asta>\begingl
		\glpreamble\scr{No akrakon aruþ.}\\
		\fw{No akrakon aruþ.}\\
	...
	\endgl
	\a<phrase>\begingl
		\glpreamble\scr{Eson moþes elbi šus ken botra draš.}\\
		\fw{Eson moþes elbi šus ken botra draš.}\\
		\phnm{e\pstrs son mo\sstrs θes el\pstrs bi \pstrs ʃus ken bot\pstrs ra \pstrs draʃ}//
...
\index[tavonic]{morphological typology!exponence|(}
...
\ex<ex:tvk-exponence>
	\begingl
		\glpreamble\scr{Nan tavotik one vi?}\\
		\fw{Nan tavotik one vi?}\\
...
Example~\getref{ex:tvk-exponence} includes one derivational morpheme and three inflectional morphemes attached to the roots \scr{tavo} \fw{tavo} and \scr{oneli} \fw{oneli}, two of which are polyexponential. The preposition \scr{nan} \fw{nan} is a polyexponential morpheme that identifies the preceding noun's gender (animate), number (plural), and topicality. The affix -\scr{tik} -\fw{tik}, a diminutive that derives the word \defn{child} from the root \defn{person}, is a monoexponential derivational suffix. The single-letter suffix -\scr{e} -\fw{e} attaches to the verb to express the mood (indicative), tense (nonpast), and aspect (imperfective). Finally, the word \scr{vi} \fw{vi} is a monoexponential interrogative clitic that turns the sentence into a question.

Noun prepositions can additionally encode case. In example~\getref{ex:tvk-exponence}, the noun \scr{tavotik} \fw{tavotik} is inferred to be in the absolutive case despite being unmarked for it. In many other situations, this grammatical case is additionally encoded within the same polyexponential preposition. In example~\getfullref{ex:tvk-iso-nouns.phrase}, the word \scr{moþes} \fw{moþes} indicates that the noun \defn{egg} is inanimate, paucal, the topic, and in the accusative case.

One noun preposition, \scr{nut} \fw{nut} has not fully cumulated, with the noun's number being still separated into a distinct segment.

\pex<ex:tvk-noncumulated>
	\a<sg>\scr{nut} \fw{nut-∅} \phnm{nut} \gloss{\An.\Top.\Acc-\Sg}
	\a<pc>\scr{nutos} \fw{nut-os} \phnm{nu\pstrs tos} \gloss{\An.\Top.\Acc-\Pc}
	\a<pl>\scr{nuton} \fw{nut-on} \phnm{nu\pstrs ton} \gloss{\An.\Top.\Acc-\Pl}
\xe
...
\pex<ex:tvk-cumulated>
	...
	\beginsubsub
		\b{i.}\scr{ða} \fw{ða} \phnm{ða} \gloss{\In.\Sg.\Erg}
		\b{ii.}\scr{ðes} \fw{ðes} \phnm{ðes} \gloss{\In.\Pc.\Erg}
		\b{iii.}\scr{dun} \fw{dun} \phnm{dun} \gloss{\In.\Pl.\Erg}
	\endsubsub
	\a<itd>Inanimate Topic Dative
	\beginsubsub
		\b{i.}\scr{moǩ} \fw{moǩ} \phnm{mox} \gloss{\In.\Sg.\Top.\Dat}
		\b{ii.}\scr{mekos} \fw{mekos} \phnm{me\pstrs kos} \gloss{\In.\Pc.\Top.\Dat}
		\b{iii.}\scr{nikun} \fw{nikun} \phnm{ni \pstrs kun} \gloss{\In.\Pl.\Top.\Dat}
	\endsubsub
\xe
...
\index[tavonic]{morphological typology!flexivity|(}
...
In example~\getref{ex:tvk-flex-nouns}, both \scr{bilt} \fw{bilt} and \scr{eðer} \fw{eðer} are marked for the paucal number and the absolutive case, but because \scr{bilt} \fw{bilt} is animate and \scr{eðer} \fw{eðer} is inanimate, the shape of the prepositions are entirely different.
...
Adjectives also show flexivity since they decline to match the gender of the noun they modify. Each adjective has a distinct animate and inanimate form, with animate adjectives ending in -\scr{a} -\fw{a}, -\scr{i} -\fw{i}, or -\scr{u} -\fw{u} and inanimate adjectives ending in -\scr{e} -\fw{e} or -\scr{o} -\fw{o}.

\pex<ex:tvk-flex-adjectives>
	\a<animate>\begingl
		\glpreamble\scr{su frandi bilt}\\
		\fw{su frandi bilt}\\
...
In example~\getref{ex:tvk-flex-adjectives}, the form of \scr{frandi} \fw{frandi} changes depending on whether it is modifying an animate noun like \scr{bilt} \fw{bilt} or an inanimate noun like \scr{eðer} \fw{eðer}.

Verbs are divided into three distinct conjugation classes, each identified by the infinitive form. Class I verb infinitives end in -\scr{ali} -\fw{ali}, class II verb infinitives end in -\scr{eli} -\fw{eli}, and class III verb infinitives end in -\scr{uli} -\fw{uli}.

\pex<ex:tvk-flex-verbs>
	\a<cl1>Class I: \scr{bruþatali} \fw{bruþat-ali} \phnm{bru.θa\pstrs ta.li} \defn{to handle} \gloss{handle-\Inf}
	\a<cl2>Class II: \scr{šeli} \fw{š-eli} \phnm{\pstrs ʃe.li} \defn{to run} \gloss{run-\Inf}
	\a<cl3>Class III: \scr{teguli} \fw{teg-uli} \phnm{te\pstrs gu.li} \defn{to worry} \gloss{worry-\Inf}
\xe

Beyond just the form of the infinitive, the verb's class determines the entire conjugation paradigm for that verb.

\pex<ex:tvk-flex-verbs>
	\a<cl1>Class I: \scr{bruþatabe} \fw{bruþat-abe} \phnm{bru.θa\pstrs ta.be} \defn{handling} \gloss{handle-\Act.\Ptcp}
	\a<cl2>Class II: \scr{šiba} \fw{š-iba} \phnm{\pstrs ʃi.ba} \defn{running} \gloss{run-\Act.\Ptcp}
	\a<cl3>Class III: \scr{tegube} \fw{teg-ube} \phnm{te\pstrs gu.be} \defn{worrying} \gloss{worry-\Act.\Ptcp}
\xe

As shown in example~\getref{ex:tvk-flex-verbs}, the same inflection takes a different form when attached to a verb of a different class. To form the active participle, \scr{bruþatali} \fw{bruþatali} becomes \scr{bruþatabe} \fw{bruþatabe} and \scr{teguli} \fw{teguli} becomes \scr{tegube} \fw{tegube}. Following this pattern, one might expect \scr{šeli} \fw{šeli} to become \ungr\scr{šebe} \ungr\fw{šebe}, but instead it becomes \scr{šiba} \fw{šiba}.

\index[tavonic]{morphological typology!flexivity|)}

\subsection{Synthesis}
\label{subsec:tvk-synthesis}
\index[tavonic]{morphological typology!synthesis|(}
...
\index[tavonic]{morphological typology!processes|(}
...
\index[tavonic]{morphological typology!processes!suffixation|(}
...
\pex<ex:tvk-verbs-sfxs>
	...
	\endgl
	\a<sbjv>\begingl
		\glpreamble\scr{Monsa ufut oþ nikis.}\\
		\fw{Monsa ufut oþ nikis.}\\
	...
	\endgl
	\a<i:imp>\begingl
		\glpreamble\scr{Mi þro akrorganta.}\\
		\fw{Mi þro akrorganta.}\\
	...
	\endgl
	\a<neg>\begingl
		\glpreamble\scr{Mana kantenta bo.}\\
		\fw{Mana kantenta bo.}\\
...
As discussed in \autoref{subsec:tvk-synthesis}, although the particle \scr{bo} \fw{bo} is a separate phonological word, it functions syntactically as a suffix. This is shown in example~\getfullref{ex:tvk-verbs-sfxs.neg} where it attaches to the verb \scr{kantenta} \fw{kantenta} to negate it.

Suffixes are also present on adjectives, though only minimally. Adjectives take one of two vowel endings to mark the gender of its referent, with animate adjectives ending in -\scr{i} -\fw{i}, -\scr{a} -\fw{a}, or -\scr{u} -\fw{u} and inanimate adjectives ending in -\scr{e} -\fw{e} or -\scr{o} -\fw{o}.

\pex<ex:tvk-adjective-sfxs>
	\a<ae>\scr{ablunga} \fw{ablunga} \phnm{ab.lun\pstrs ga} \gloss{\An} vs. \scr{ablunge} \fw{ablunge} \phnm{ab.lun\pstrs ge} \gloss{\In} \defn{catlike}
	\a<io>\scr{akrandi} \fw{akrandi} \phnm{ak.ran\pstrs di} \gloss{\An} vs. \scr{akrando} \fw{akrando} \phnm{ak.ran\pstrs do} \gloss{\In} \defn{writable}
	\a<ao>\scr{bruþatla} \fw{bruþatla} \phnm{bru.θat\pstrs la} \gloss{\An} vs. \scr{bruþatlo} \fw{bruþatlo} \phnm{bru.θat\pstrs lo} \gloss{\In} \defn{manual}
	\a<uo>\scr{fraþru} \fw{fraþru} \phnm{fraθ\pstrs ru} \gloss{\An} vs. \scr{fraþro} \fw{fraþro} \phnm{fraθ\pstrs ro} \gloss{\In} \defn{observant}
\xe
...
\pex<ex:tvk-derivation-sfxs>
	\a<root1>\scr{frali} \fw{frali} \phnm{\pstrs fra.li} \defn{to see}
	\a<der11>\scr{fravem} \fw{fravem} \phnm{fra\pstrs vem} \defn{sight}
	\a<der12>\scr{fravemitla} \fw{fravemitla} -\scr{o} -\fw{o} \phnm{fra.vem.it\pstrs la} \defn{visual}
	\a<root2>\scr{onaš} \fw{onaš} \phnm{o\pstrs naʃ} \defn{rug}
	\a<der21>\scr{onašuli} \fw{onašuli} \phnm{o.na\pstrs ʃu.li} \defn{to place}
	\a<der22>\scr{onašinsuli} \fw{onašinsuli} \phnm{o.na.ʃin\pstrs su.li} \defn{to re-place}
\xe

In example~\getfullref{ex:tvk-derivation-sfxs.der22}, the -\scr{ins} -\fw{ins} affix may not immediately appear to be a suffix, however it should be noted that it is being attached to the end of the \emph{stem} of the word, which is \scr{onaš}- \fw{onaš}-, prior to the verb's infinitive ending -\scr{uli} -\fw{uli}, which is an \emph{inflectional} suffix.

\index[tavonic]{morphological typology!processes!suffixation|)}

\subsection{Cliticization}
\label{subsec:tvk-cliticization}
\index[tavonic]{morphological typology!processes!cliticization|(}
...
\pex<ex:tvk-noun-clitics>
	\a<simple>\begingl
		\glpreamble\scr{Mod nas oko fra.}\\
		\fw{Mod nas oko fra.}\\
	...
	\endgl
	\a<adjective>\begingl
		\glpreamble\scr{Mod nas urda oko fra.}\\
		\fw{Mod nas urda oko fra.}\\
	...
	\a<possessedpro>\begingl
		\glpreamble\scr{Mod nas tesar urda oko fra.}\\
		\fw{Mod nas tesar urda oko fra.}\\
	...
	\a<possessednoun>\begingl
		\glpreamble\scr{Mod nas su eson urda oko fra.}\\
		\fw{Mod nas su eson urda oko fra.}\\
		\phnm{\pstrs mod nas su e\pstrs son ur\pstrs da o\pstrs ko \pstrs fra}//
...
Notice in example~\getref{ex:tvk-noun-clitics} how the particle \scr{nas} \fw{nas} directly precedes the entire noun phrase, even when separated from the head noun by an adjective (\getfullref{ex:tvk-noun-clitics.adjective}), a pronoun (\getfullref{ex:tvk-noun-clitics.possessedpro}), and even another modifying noun and its preposition (\getfullref{ex:tvk-noun-clitics.possessednoun}).
...
\pex<ex:tvk-clitic-reduction>
	\a<le>\scr{le eðer} \fw{le eðer} → \scr{l'eðer} \fw{l'eðer} \phnm{le\pstrs ðer} \defn{pens} \gloss{\In.\Pc.\Abs-pen}
	\a<mati>\scr{mati inam} \fw{mati inam} → \scr{mat'inam} \fw{mat'inam} \phnm{ma.ti\pstrs nam} \defn{location} \gloss{\In.\Sg.\Top.\Acc-location}
	\a<no>\scr{no oko} \fw{no oko} → \scr{n'oko} \fw{n'oko} \phnm{no\pstrs ko} \defn{dog} \gloss{\An.\Sg.\Top-pen}
	\a<su>\scr{su urda ablu} \fw{su urda ablu} → \scr{s'urda ablu} \fw{s'urda ablu} \phnm{sur\pstrs da ab\pstrs lu} \defn{of the protected cat} \gloss{\An.\Sg.\Gen-protected-\An{} cat}
\xe

This phonological reduction occurs no matter whether the following word is the noun the preposition is modifying or not. For example, notice in example~\getfullref{ex:tvk-clitic-reduction.su} that the preposition attaches itself to \scr{urda} \fw{urda} even though it is an adjective modifying the noun \scr{ablu} \fw{ablu}.

The other main example of cliticization is the particle \scr{vi} \fw{vi}. It is used to ask questions and is most often added at the end of a sentence after the verb, as shown in example~\getref{ex:tvk-clitic-question}.

\ex<ex:tvk-clitic-question>
	\begingl
		\glpreamble\scr{No šekon tu fraþru oko usu vi?}\\
		\fw{No šekon tu fraþru oko usu vi?}\\
...
\pex<ex:tvk-clitic-question-focus>
	\a<erg>\begingl
		\glpreamble\scr{No šekon vi tu fraþru oko usu?}\\
		\fw{No šekon vi tu fraþru oko usu?}\\
		\phnm{no ʃe\pstrs kon vi tu fraθ\pstrs ru o\pstrs ko u\pstrs su}//
		\gla no= šekon =vi tu= fraþr-u oko us-u//
		\glb \An.\Sg.\Top{}= runner =\Q{} \An.\Sg.\Acc{}= observant-\An{} dog have-\Ind.\Npst.\Ipfv//
		\glft\defn{Is it the runner who has an observant dog?}//
	\endgl
	\a<acc>\begingl
		\glpreamble\scr{No šekon tu fraþru vi oko usu?}\\
		\fw{No šekon tu fraþru vi oko usu?}\\
		\phnm{no ʃe\pstrs kon tu fraθ\pstrs ru vi o\pstrs ko u\pstrs su}//
		\gla no= šekon tu= fraþr-u =vi oko us-u//
		\glb \An.\Sg.\Top{}= runner \An.\Sg.\Acc{}= observant-\An{} =\Q{} dog have-\Ind.\Npst.\Ipfv//
		\glft\defn{Is it an \emph{observant} dog the runner has?}//
	\endgl
	\a<adj>\begingl
		\glpreamble\scr{No šekon tu fraþru oko vi usu?}\\
		\fw{No šekon tu fraþru oko vi usu?}\\
		\phnm{no ʃe\pstrs kon tu fraθ\pstrs ru o\pstrs ko vi u\pstrs su}//
		\gla no= šekon tu= fraþr-u oko =vi us-u//
		\glb \An.\Sg.\Top{}= runner \An.\Sg.\Acc{}= observant-\An{} dog =\Q{} have-\Ind.\Npst.\Ipfv//
...
\label{sec:tvk-locus}
\index[tavonic]{morphological typology!locus of marking|(}
...
\pex<ex:tvk-depmarking-possessive>
	...
	\begin{minipage}[t]{0.5\remaining}
		...
		\end{forest}
	\end{minipage}
	\a<n>\begin{minipage}[t]{0.5\remaining}
		\begingl
			\glpreamble\scr{su esobiš botra}\\
			\fw{su esobiš botra}\\
...
In example~\getfullref{ex:tvk-depmarking-possessive.pn}, \defn{you} are grammatically in possession of \scr{botra} \fw{botra} \defn{wife}; the possessee forms the head of the phrase while it is modified by the possessor, which receives the genitive inflection. In example~\getfullref{ex:tvk-depmarking-possessive.n}, \scr{botra} \fw{botra} is still the possessee and thus the head of the phrase while the genitive is marked on the dependent, \scr{esobiš} \fw{esobiš} \defn{patriot}, using a noun preposition.

\langtvk{} also shows dependent marking when modifying nouns with adjectives.

\ex<ex:tvk-depmarking-adjective>
	\remainex\begin{minipage}[t]{0.5\remaining}
		\begingl
			\glpreamble\scr{ran ǩalonža botra}\\
			\fw{ran ǩalonža botra}\\
			\phnm{ran xa.lon\pstrs ʒa bot\pstrs ra}//
			\gla\textbf{ran}= ǩalonž-\textbf{a} botra//
			\glb\textbf{\An}.\Pl.\Abs= husbandless-\textbf{\An} woman//
			\glft\defn{husbandless women}//
		\endgl
	\end{minipage}
	\begin{minipage}[t]{0.5\remaining}
		\begin{forest} dotted tier
			[\scr{botra}
				[\scr{\textbf{ran}}, name=prepmod
					[\fw{\textbf{ran}}, name=prep]
				]
				[\scr{ǩalonž\textbf{a}}, name=adj
					[\fw{ǩalonž\textbf{a}}, name=dependent]
				]
				[\fw{botra}, name=head]
			]
...
In example~\getref{ex:tvk-depmarking-adjective}, while the head noun \scr{botra} \fw{botra} is marked for animacy with \scr{ran} \fw{ran}, the dependent modifying adjective \scr{ǩalonža} \fw{ǩalonža} \defn{husbandless} takes the -\scr{a} -\fw{a} animate ending to match.
...
\pex<ex:tvk-depmarking-verbs>
	\a<intranspn>\remainpex\begin{minipage}[t]{0.5\remaining}
		\begingl
			\glpreamble\scr{Mon usuš.}\\
			\fw{Mon usuš.}\\
			\phnm{\pstrs mon u\pstrs suʃ}//
			\gla \textbf{mon} us-uš//
			\glb \Fps.\textbf{\Top} sing-\Ind.\Npst.\Rtsp//
			\glft\defn{I have sung.}//
		\endgl
	\end{minipage}
	\begin{minipage}[t]{0.5\remaining}
		\begin{forest} dotted tier
	...
	\a<transpn>\begin{minipage}[t]{0.5\remaining}
		\begingl
			\glpreamble\scr{Mon þet kante.}\\
			\fw{Mon þet kante.}\\
			\phnm{\pstrs mon \pstrs θet kan\pstrs te}//
			\gla \textbf{mon} \textbf{þet} kant-e//
			\glb \Fps.\textbf{\Top} \Sps.\textbf{\Acc} thank-\Ind.\Npst.\Imp//
			\glft\defn{I thank you.}//
		\endgl
	\end{minipage}
	\begin{minipage}[t]{0.5\remaining}
		\begin{forest} dotted tier
			[\scr{kante}
				[\scr{\textbf{mon}}
					[\fw{\textbf{mon}}, name=dependentabs]
				]
				[\scr{\textbf{þet}}
					[\fw{\textbf{þet}}, name=dependentacc]
				]
				[\fw{kante}, name=head]
			]
			\node at (head.south) {\textsc{\tiny head}};
	...
	\a<intransn>\begin{minipage}[t]{0.5\remaining}
		\begingl
			\glpreamble\scr{Nas arb usuš.}\\
			\fw{Nas arb usuš.}\\
			\phnm{nas \pstrs arb u\pstrs suʃ}//
			\gla \textbf{nas}= arb us-uš//
			\glb \An.\Pc.\textbf{\Top}= bird sing-\Ind.\Npst.\Rtsp//
			\glft\defn{The birds have sung.}//
		\endgl
	\end{minipage}
	\begin{minipage}[t]{0.5\remaining}
		\begin{forest} dotted tier
	...
	\end{minipage}
	\a<transn>\begingl
		\glpreamble\scr{Do su botra ǩalo mi tavofre alma frak.}\\
		\fw{Do su botra ǩalo mi tavofre alma frak.}\\
		\phnm{do su bot\pstrs ra xa\pstrs lo mi ta.vof\pstrs re al\pstrs ma \pstrs frak}//
		...
		\glft\defn{The woman's husband saw the inhabited house.}//
	\endgl\\
	\begin{forest} dotted tier
		[\scr{frak}
			[\scr{ǩalo}
				...
				[\fw{ǩalo}, name=dependenterg]
			]
			[\scr{alma}
				[\scr{\textbf{mi}}, name=prepmod
					[\fw{\textbf{mi}}]
				]
				[\scr{tavofre}, name=adj
					[\fw{tavofre}]
				]
				[\fw{alma}, name=dependentacc]
			]
			[\fw{frak}, name=head]
		]
		\node at (head.south) {\textsc{\tiny head}};
...
In example~\getfullref{ex:tvk-depmarking-verbs.intranspn}, the pronoun \scr{mon} \fw{mon} is declined to indicate it is the topic of the sentence while the verb \scr{usuš} \fw{usuš}, despite conjugating for mood, tense, and aspect, is not marked for this role. Example~\getfullref{ex:tvk-depmarking-verbs.transpn} similarly marks the two pronouns \scr{mon} \fw{mon} and \scr{þet} \fw{þet} for their roles in the sentence as topic and object while the verb \scr{kante} \fw{kante} does not inflect to indicate these roles. When nouns are used instead of pronouns, as in examples~\getfullref{ex:tvk-depmarking-verbs.intransn}--\getref{ex:tvk-depmarking-verbs.transn}, the nouns are marked for their grammatical role by their prepositions, their own dependents, while the head verb remains unmarked for these roles.
