...
\subsection{Proper Names and Common Nouns}
...
\pex<ex:tvk-noun-proper>\hspace{1.72em}\remainpex\begin{minipage}[t]{0.5\remaining}\strong{Proper names}\end{minipage}\begin{minipage}[t]{0.5\remaining}\strong{Common nouns}\end{minipage}
	...
	\a<relative>\ljudge{\ques}\remainpex\begin{minipage}[t]{0.5\remaining}
		\scr{mor suk fraǧ put Ronne}\\
		\fw{mor suk fraǧ put Ronne}\\
		\defn{the Ronne that I had seen}
	\end{minipage}
	\begin{minipage}[t]{0.5\remaining}
		\scr{mor gake fraǧ put fild}\\
		\fw{mor gake fraǧ put fild}\\
		\defn{the doll that I had seen}
	\end{minipage}
\xe

In all of the examples above, the treatment of the common noun \fw{fild} is perfectly acceptable. However, whenever the noun is modified in some way, such as by specifying the number, adding a descriptive adjective, or adding an identifying relative clause, such as in examples~\getfullref{ex:tvk-noun-proper.number}–\getref{ex:tvk-noun-proper.relative}, the treatment of the proper name \fw{Ronne} is questionable. These expressions are possible, but the context must be such that the specific referent is not automatically identifiable, which is unusual for proper names. Meanwhile, using the paucal or plural form with a proper name, such as in example~\getfullref{ex:tvk-noun-proper.paucal}, changes the meaning to signify the proper name \emph{and their associates}. See more about the associative plural in \autoref{subsec:tvk-nouns-number}.
...
However, the subject of certain transitive verbs will also take the intransitive case if the semantic meaning of the verb is stative. See \autoref{subsubsec:tvk-nouns-dative} for more information on this use of the Dative, and see \autoref{subsubsec:tvk-verb-activeness} for more information on stative verbs. Since it is used in these situations, and since the intransitive is the citation form, the case is normally referred to as the absolutive case, even when used intransitively. These terms are interchangeable.
...
The ergative case marks a noun or noun phrase that serves as the subject of an active transitive verb or any ditransitive verb. This means that when a verb has multiple arguments and the semantic meaning of the verb is active, the subject argument will by default by in the ergative case. See \autoref{subsubsec:tvk-verb-activeness} for more information on active verbs.
...
Certain monotransitive verbs are used with the absolutive and dative cases instead of the ergative and accusative cases. These tend to be stative verbs in which the object of the verb is unaffected by the action or there is little volition on the part of the subject. See \autoref{subsubsec:tvk-verb-activeness} for more information on stative verbs.
...
\pex<ex:tvk-noun-gen>
	\a<ergacc>\begingl
		...
		\phnm{do su zar\pstrs sa o\pstrs ko tu mos ab\pstrs lu o.ko.ta\pstrs ða}//
...
\pex<ex:tvk-noun-top-nonsubj>
	...
	\endgl
	\a<dat-ac>\begingl
		\glpreamble\scr{Do eson ti ennis n’oko draš.}\\
		\fw{Do eson ti ennis n'oko draš.}\\
		\phnm{do e\pstrs son ti e\pstrs\gem{n}is no\pstrs ko \pstrs draʃ}//
...
\pex<ex:tvk-dem-pronoun-pl>
	\a<pc>\begingl
		...
		\fw{Mor kos ðle usu.}\\
		\phnm{\pstrs mor kos ðle u\pstrs su}//
		\gla mor ǩo= ðle us-u//
	...
	\a<pl>\begingl
		...
		\fw{Mor ǩun ðle usu.}\\
		\phnm{\pstrs mor xun ðle u\pstrs su}//
		\gla mor ǩo= ðle us-u//
...
Relative pronouns are used to create subordinate clauses. There are two relative pronouns: \scr{put} \fw{put} for restrictive relativization and \scr{bas} \fw{bas} for unrestrictive relativization.

\pex<ex:tvk-rel-pronoun>
	\a<rtrv>\begingl
		\glpreamble\scr{Do sur ke arb frak put ǩalo ti ennis ǧirak.}\\
		\fw{Do sur ke arb frak put ǩalo ti ennis ǧirak.}\\
		\phnm{do \pstrs sur ke \pstrs arb \pstrs frak put xa\pstrs lo ti en\pstrs nis ɣi\pstrs rak}//
		...
		\glft\defn{The man who saw the bird threw the ball.}//
	\endgl
	\a<nrtrv>\begingl
		\glpreamble\scr{Sur ke arb frak bas do ǩalo ti ennis ǧirak.}\\
		\fw{Sur ke arb frak bas do ǩalo ti ennis ǧirak.}\\
		\phnm{\pstrs sur ke \pstrs arb \pstrs frak bas do xa\pstrs lo ti en\pstrs nis ɣi\pstrs rak}//
...
The restrictive relative clause defines or restricts the referent with further information. Example~\getfullref{ex:tvk-rel-pronoun.rtrv} restricts the referent to specifically the man \emph{who saw the bird} instead of a different man. Meanwhile, the non-restrictive relative clause provides supplemental information about the referent without further defining or restricting. Example~\getfullref{ex:tvk-rel-pronoun.nrtrv} provides further information that the man saw the bird, but does not use that as an identifying quality.

The relative pronouns do not decline in any way. Instead, a personal pronoun is used, declined to show the role of the referent within the embedded clause. In examples~\getfullref{ex:tvk-rel-pronoun.rtrv}–\getref{ex:tvk-rel-pronoun.nrtrv}, the referent \scr{ǩalo} \fw{ǩalo} \defn{man} is referred to in the embedded clause using the pronoun \scr{sur} \fw{sur} to show the absolutive case because the man is the actor within the embedded clause, the one seeing the bird. A more literal translation of example~\getfullref{ex:tvk-rel-pronoun.rtrv} would be something like \defn{The man [who he saw the bird] threw the ball.} Similarly, a more literal translation of example~\getfullref{ex:tvk-rel-pn-order.rtrv} would be something like \defn{The man wrote the letters [that I saw them] to his wife.}

\pex<ex:tvk-rel-pn-order>
	\a<rtrv>\begingl
		\glpreamble\scr{Do ǩalo ten mor goken frak put akrapis ke šus botra eðerak.}\\
		\fw{Do ǩalo ten mor goken frak put akrapis ke šus botra eðerak.}\\
		\phnm{do xa\pstrs lo ten \pstrs mor go\pstrs ken \pstrs frak put ak.ra\pstrs pis ke \pstrs ʃus bot\pstrs ra e.ðe\pstrs rak}//
		...
		\glft\defn{The man wrote the letters that I saw to his wife.}//
	\endgl
	\a<nrtrv>\begingl
		\glpreamble\scr{Do ǩalo mor goken frak bas ten akrapis ke šus botra eðerak.}\\
		\fw{Do ǩalo mor goken frak bas ten akrapis ke šus botra eðerak.}\\
		\phnm{do xa\pstrs lo \pstrs mor go\pstrs ken \pstrs frak bas ten ak.ra\pstrs pis ke \pstrs ʃus bot\pstrs ra e.ðe\pstrs rak}//
		...
		\glft\defn{The man wrote the letters, which I saw, to his wife.}//
	\endgl
\xe

The location of the declining clitic for the head noun of the relative clause depends on whether the relative pronoun is restrictive or non-restrictive. When the pronoun is restrictive, as in examples~\getfullref{ex:tvk-rel-pronoun.rtrv} and \getfullref{ex:tvk-rel-pn-order.rtrv}, the declining clitic is placed before the entire relative clause, since the embedded clause is serving as a descriptor and thus a part of the noun phrase as a whole. On the other hand, when the pronoun is non-restrictive, as in examples~\getfullref{ex:tvk-rel-pronoun.nrtrv} and \getfullref{ex:tvk-rel-pn-order.nrtrv}, the embedded clause is merely supplemental information separate from the noun phrase itself, and so the clitic is placed after the relative pronoun immediately before the head noun.
...
\pex<ex:tvk-ind-dtm>
	\a<boza>\begingl
		\glpreamble\scr{Tu elto ǩo boza tavotik dranta.}\\
		\fw{Tu elto ǩo boza tavotik dranta.}\\
		\phnm{tu el\pstrs to xo bo\pstrs za ta.vo\pstrs tik \pstrs dran.ta}//
		\gla tu= elto ǩo= boza tavotik dr-anta//
		\glb \An.\Sg.\Acc= water \An.\Sg.\Dat= any child give-\Imp//
		\glft\defn{Give the water to any child.}//
	\endgl
	\a<kirt>\begingl
		\glpreamble\scr{Tu kirt elto suk dranta.}\\
		\fw{Tu kirt elto suk dranta.}\\
		\phnm{tu kirt el\pstrs to \pstrs suk \pstrs dran.ta}//
		\gla tu= kirt elto suk dr-anta//
		\glb \An.\Sg.\Acc= no water \Tps.\An.\Dat= give-\Imp//
		\glft\defn{Give no water to him.}//
	\endgl
\xe
...
\pex<ex:tvk-ind-prn>
	\a<boza>\begingl
		\glpreamble\scr{Tu elto ǩo boza dranta.}\\
		\fw{Tu elto ǩo boza dranta.}\\
		\phnm{tu el\pstrs to xo bo\pstrs za \pstrs dran.ta}//
		\gla tu= elto ǩo= boza dr-anta//
		\glb \An.\Sg.\Acc= water \An.\Sg.\Dat= anyone give-\Imp//
		\glft\defn{Give the water to any(one).}//
		\endgl
	\a<kirt>\begingl
		\glpreamble\scr{Tu kirt suk dranta.}\\
		\fw{Tu kirt suk dranta.}\\
		\phnm{tu kirt \pstrs suk \pstrs dran.ta}//
		\gla tu= kirt suk dr-anta//
		\glb \An.\Sg.\Acc= none \Tps.\An.\Dat= give-\Imp//
		\glft\defn{Give none/nothing to him.}//
	\endgl
\xe

As shown in these contextless examples~\getfullref{ex:tvk-ind-prn.boza}–\getref{ex:tvk-ind-prn.kirt}, an indefinite pronoun may be translated a few different ways into English. Discourse context generally provides the clues necessary to understand them more precisely. For example, the \fw{ǩo} of example~\getfullref{ex:tvk-ind-prn.boza} indicates that the pronoun \fw{boza} is animate and singular. The rest of the discourse would provide the remaining context.

\section{Verbs}
\label{sec:tvk-verbs}

Verbs are the primary class of words that carries inflection, showing information such as tense, aspect, and more. The following sections examine \langtvk{} verbs in more detail.

\subsection{Types of Verbs}
\label{subsec:tvk-verb-types}

There are a number of ways verbs in \langtvk{} can be divided into groups, each of which treats its arguments differently. Verbs differ in valency, meaning that some verbs may have one argument (intransitive) while others have two (transitive), three (ditransitive), a variable number (ambitransitive), or even no argument at all (impersonal). Verbs also differ in activeness, with some being active and others stative. There are also three (largely arbitrary) classes of verbs that govern the shape of the inflected morphology.

\subsubsection{Verb Valency}
\label{subsubsec:tvk-verb-valency}

Verbs can be grouped by valency into intransitive, transitive, ambitransitive, and ditransitive depending on how many arguments it takes.

Intransitive verbs are those that have only one argument. These tends to be verbs describing involuntary processes, bodily functions, motion, and position, as well as weather verbs.

\pex<ex:tvk-vb-val-intr>
	\a<aruli>\begingl
		\glpreamble\scr{Mos persa arum.}\\
		\fw{Mos persa arum.}\\
		\phnm{mos per\pstrs sa a\pstrs rum}//
		\gla ∅= mos persa ar-um//
		\glb \An.\Sg.\Abs= \Fps.\Gen{} sister stand-\Ind.\Pst.\Ipfv//
		\glft\defn{My sister was standing.}//
	\endgl
	\a<puzali>\begingl
		\glpreamble\scr{Sur puzak.}\\
		\fw{Sur puzak.}\\
		\phnm{sur pu\pstrs zak}//
		\gla sur puz-ak//
		\glb \Tps.\An.\Abs{} fall-\Ind.\Pst.\Pfv//
		\glft\defn{She fell.}//
	\endgl
	\a<orðali>\begingl
		\glpreamble\scr{Sur bag orða.}\\
		\fw{Sur bag orða.}\\
		\phnm{sur bag or\pstrs ða}//
		\gla sur bag orð-a//
		\glb \Tps.\An.\Abs{} now cry-\Ind.\Npst.\Ipfv//
		\glft\defn{Now she is crying.}//
	\endgl
\xe

Weather verbs are a special class of intransitive verbs in that generally the only argument it can take is a dummy pronoun, \fw{gir}. In this way, weather verbs really have zero valency, though morphologically they are treated as though they are intransitive.

\pex<ex:tvk-vb-val-weather>
	\a<rain>\begingl
		\glpreamble\scr{Gir olvaš.}\\
		\fw{Gir olvaš.}\\
		\phnm{gir ol\pstrs vaʃ}//
		\gla gir olv-aš//
		\glb \Tps.\In.\Abs{} rain-\Ind.\Npst.\Rtsp//
		\glft\defn{It has rained.}//
	\endgl
	\a<snow>\begingl
		\glpreamble\scr{Gir irbe.}\\
		\fw{Gir irbe.}\\
		\phnm{gir ir\pstrs be}//
		\gla gir irb-e//
		\glb \Tps.\In.\Abs{} snow-\Ind.\Npst.\Ipfv//
		\glft\defn{It is snowing.}//
	\endgl
\xe

Transitive verbs are those that have two arguments, generally an agent and a patient. These tend to be verbs describing actions and action-processes, factives, cognition, sensation, and emotion.

\pex<ex:tvk-vb-val-tran>
	\a<worry>\begingl
		\glpreamble\scr{Mod ke mosar okotik tegu.}\\
		\fw{Mod ke mosar okotik tegu.}\\
		\phnm{\pstrs mod ke mo\pstrs sar o.ko\pstrs tik te\pstrs gu}//
		\gla mod ke= mosar okotik teg-u//
		\glb \Fps.\An.\Abs{} \Sg.\An.\Dat{}= \Fpc.\An.\Gen{} puppy worry.about-\Ind.\Npst.\Ipfv//
		\glft\defn{I am worried about our puppy.}//
	\endgl
	\a<protect>\begingl
		\glpreamble\scr{Do avo sut urdatem.}\\
		\fw{Do avo sut urdatem.}\\
		\phnm{do a\pstrs vo \pstrs sut ur.da\pstrs tem}//
		\gla do= avo sut urdat-em//
		\glb \Sg.\An.\Erg= father \Tps.\An.\Acc{} protect-\Ind.\Pst.\Ipfv//
		\glft\defn{Father protected him.}//
	\endgl
	\a<hear>\begingl
		\glpreamble\scr{Ter suk puzaþ emrina vi?}\\
		\fw{Ter suk puzaþ emrina vi?}\\
		\phnm{\pstrs ter \pstrs suk pu\pstrs zaθ em.ri\pstrs na vi}//
		\gla ter suk puz-aþ emrin-a =vi//
		\glb \Sps.\Abs{} \Tps.\An.\Dat{} cry-\Ind.\Npst.\Prg{} hear-\Ind.\Npst.\Ipfv{} =\Int//
		\glft\defn{Do you hear him crying?}//
	\endgl
\xe

Ambitransitive verbs are those that can alternatively take one or two arguments. The subject of the verb is mandatory, while the direct object is optional. With stative verbs, the change is managed through simple omission, however with active verbs, this requires switching the subject from the Ergative to the Absolutive. See \autoref{subsubsec:tvk-verb-activeness} for more information on verb activeness.

\textit{Examples}

Ditransitive verbs are those that take three arguments, including an indirect object. These tend to be verbs describing action-processes.

\textit{Examples}

\subsubsection{Verb Activeness}
\label{subsubsec:tvk-verb-activeness}
