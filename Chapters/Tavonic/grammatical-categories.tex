...
The absolutive case marks a noun or noun phrase that serves as the subject of an intransitive verb. This means that when a verb has only a single argument, that argument will by default be in the absolutive case. That is true whether the subject is serving like an agent as in words like \fw{šeli} \defn{to run} or \fw{ufuli} \defn{to sing} or when the subject is serving more like a patient as in words like \fw{orðali} \defn{to fall}.
...
\pex<ex:tvk-noun-abs-sg>
	\a<an>\begingl
		...
		\gla ∅= alum uldet-eš//
		\glb \An.\Sg.\Abs= cloud change-\Ind.\Npst.\Rtsp//
	...
	\a<in>\begingl
		...
		\gla ∅= almaþ uldet-eš//
		\glb \In.\Sg.\Abs= village change-\Ind.\Npst.\Rtsp//
		\glft\defn{The village has changed.}//
	\endgl
\xe

The absolutive case is frequently used with postpositions to indicate static location, for example being placed at, on, or in something.
...
The ergative case marks a noun or noun phrase that serves as the subject of a transitive verb. This means that when a verb has multiple arguments, the subject argument will by default by in the ergative case. That is true whether the subject is serving like an agent as in words like \fw{eðerali} \defn{to write} or \fw{okotali} \defn{to hunt} or when the subject is serving more like a patient as in words like \fw{frali} \defn{to see}.

\pex<ex:tvk-noun-erg>
	\a<a1>\begingl
		\glpreamble\fw{Do Tlunda ti akrapis eðeraš.}\\
		\phnm{do \pstrs tlun\pstrs da ti ak.ra\pstrs pis e.ðe\pstrs raʃ}//
	...
	\endgl
	\a<p>\begingl
		\glpreamble\fw{Din avo ten usudir fraǧ.}\\
		\phnm{din a\pstrs vo ten u.su\pstrs dir \pstrs fraɣ}//
		\gla din= avo ten= usudir fr-aǧ//
		\glb \An.\Pl.\Erg= father \In.\Pl.\Acc= basket see-\Ind.\Pst.\Rtsp//
		\glft\defn{The father and his associates had seen the baskets.}//
	\endgl
\xe
...
The accusative case marks a noun or noun phrase that serves as the direct object of a transitive verb. In most situations, the direct object is the entity being acted upon by the subject, as in words like \fw{onašuli} \defn{to place} or \fw{uldetuli} \defn{to change}, but in some cases may merely be the focus of a state, as in words like \fw{frali} \defn{to see} or \fw{keðali} \defn{to admire}.
...
\pex<ex:tvk-noun-acc>
	...
	\endgl
	\a<s>\begingl
		\glpreamble\fw{Din urdaton ton ufukon keðam.}\\
		\phnm{din ur.da\pstrs ton ton u.fu\pstrs kon ke\pstrs ðam}//
...
The dative case marks a noun or noun phrase that serves as the indirect object of a ditransitive verb. It may be a recipient of an action or it may be the entity for whose benefit the action is taken.
